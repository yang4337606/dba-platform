...
from .models import AWRReport, AWRAnalysis, KnowledgeBase, GitHubProject
...
def dashboard():
    ...
    analysis_count = AWRAnalysis.query.count()
    kb_count = KnowledgeBase.query.filter_by(is_active=True).count()
    recent_reports = AWRReport.query.order_by(AWRReport.created_at.desc()).limit(5).all()
    recent_analyses = AWRAnalysis.query.order_by(AWRAnalysis.created_at.desc()).limit(5).all()
